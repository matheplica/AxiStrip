    ...
    if (port != null) {
        ...
        if (port.available() > 0) {                // Ok, we have something... may be complete (terminated) or not.
            ...
            while (loop) {
                ...
                if (b != null)  a += b;                              // Combine with the found ones.
                else loop = false;                        // Nothing to do anymore. We try the next cycle.
            }
            if (a.length() > 0) {
                ...
                for (String d : c) {
                    ...
                    if (d.startsWith("1")) axi.setPenStatus(1);
                    else if (d.startsWith("0")) axi.setPenStatus(0);
                    else if (d.startsWith("QM")) isRunning = (d.charAt(5)=='0' || d.charAt(7)=='0') ? false : true;
                    else println("AxiDraw says [" + millis() + "ms]: " + d );
                }
            }
        }
    }
